            ...
            for (var filter in params) {
                if (params.hasOwnProperty(filter) && filter.lastIndexOf(bc.filter_prefix, 0) === 0) {
                    ...
                    if (bc.filter_contexts.indexOf(filter_context) !== -1) {
                        ...
                        //if(bc.selected_filter_context_from_url == ""){
                        //    bc.selected_filter_context_from_url = filter_context;
                        //    bc.selected_filter_from_url = params[filter];
                        bc.filters.push({
...
bc.onclick = function (event, item) {
    ...
    if(!bc.filterContextFromURL){
        var url = "https://localhost:9443/portal/dashboards/iot-analytics-dashboard/filter?g_"+filteringContext+"="+filteringGroup;
        window.open(url);
    } else {
        if (item != null) {
            ...
            bc.freeze = bc.isFreeze();
            //console.log("removing: " + filteringContext + bc.selected_filter_groups[filteringContext].length);
            bc.updateURL();
            bc.update(true);
        }
    }
