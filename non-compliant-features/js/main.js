    ...
    charts: [
        {
            ...
            yTitle: "",
            mode: "group",
            colorScale: ["#e74c3c", "#d35400", "#f39c12", "#f1c40f"],
            //colorDomain: ["Non Compliant2", "No Passcode", "Non encrypted", "Unmonitored"],
            orientation: "left",
            legend: false
        }
    ],
...
//    height: $(window).width() * 0.65 > $(window).height() ? $(window).height() : $(window).width() * 0.65,
    padding: {"top": 0, "left": 150, "bottom": 0, "right": 50}
};
...
bc.fetch = function (cb) {
    ...
    $.ajax({
        url: gadgetConfig.source + "&start-index=" + bc.fromIndex + "&result-count=" + bc.count,
        method: "GET",
        success: function (response) {
            ...
            if (Object.prototype.toString.call(response) === '[object Array]' && response.length === 1) {
                ...
                if (data && data.length > 0) {
                    ...
                    div.innerHTML = 'No of violated features: ' + response[0]["totalRecordCount"];
                }
            } else {
...
bc.onclick = function (event, item) {
    if (!bc.filterContextFromURL) {
        ...
        var url = "https://localhost:9443/portal/dashboards/iot-analytics-dashboard/filter?g_" + bc.filter_context + "=" + filteringGroup;
        window.open(url);
    }
};
